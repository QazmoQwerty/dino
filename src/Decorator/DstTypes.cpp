...
namespace DST 
{
	...
	{
		if (append->isListTy())
			throw ErrorReporter::reportInternal("cannot append a list type to a list type", ERR_GENERAL);
	...
	{
		if (tys.size() < 2)
			throw ErrorReporter::reportInternal("type list must have at least 2 types", ERR_GENERAL);
