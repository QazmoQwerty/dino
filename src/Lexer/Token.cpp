...
LiteralToken<unicode_char> * createCharacterLiteralToken(unicode_string value, ErrorReporter::Position pos)	
{
	char val = 'f';
	if (value.length() == 0)
		throw ErrorReporter::report("Empty char constant", ERR_GENERAL, pos);
	if (value.length() > 1) 
	{
		if (value.length() == 2 && value[0].getValue() == ESCAPE_CHAR) 
		{
			if (value[1].getValue() == 't')
				...
				val = '\'';
				// value = unicode_string("'");
			else throw ErrorReporter::report("Too many characters in character constant", ERR_GENERAL, pos);
		}
		else throw ErrorReporter::report("Too many characters in character constant", ERR_GENERAL, pos);
	}
...
LiteralToken<float> * createFractionLiteralToken(unicode_string data, ErrorReporter::Position pos)
{
	...
	catch (exception) { throw ErrorReporter::report("Invalid fraction literal", ERR_GENERAL, pos); }
	return token;
}
...
LiteralToken<int> * createIntegerLiteralToken(unicode_string data, ErrorReporter::Position pos)
{
	...
	catch (exception) { throw ErrorReporter::report("Invalid integer literal", ERR_GENERAL, pos); }
	return token;
}
...
LiteralToken<bool> * createBooleanLiteralToken(unicode_string data, ErrorReporter::Position pos)
{
	...
	if (data == "true")
		...
		temp->_value = false;
	else throw ErrorReporter::report("Invalid boolean literal", ERR_GENERAL, pos);
	return temp;
}
